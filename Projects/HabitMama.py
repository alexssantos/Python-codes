...
def desenharArena():
    ...
    texto = font.render(str((90000-pygame.time.get_ticks())//60000)+":" +
...
def desenhaFlecha(flechas):
    ...
    for bala in flechas:
        ...
        for projectil in flechas:
            flecha_temp = pygame.transform.rotate(
                flecha, 360-projectil[0]*57.29)     # 57.29 Correction Factor ARROW_POISTION = MAMA_POSITION
...
def desenhaFim(acuraciaFinal, imagem):
    ...
    font = pygame.font.Font(None, 24)
    texto = font.render(
        "Acurácia: "+"{0:.2f}".format(acuraciaFinal)+"%", True, (0, 255, 0))
    textoRect = texto.get_rect()
...
def main():
    ...
    while not terminou:  # Loop principal do jogo
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            acuracia[1] += 1
            flechas.append([math.atan2(clickPos[1]-(coelhoPos[1]+32),
                                       clickPos[0]-(coelhoPos[0]+32)), coelhoPos[0]+32, coelhoPos[1]+32])
