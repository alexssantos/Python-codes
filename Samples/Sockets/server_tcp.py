...
while True:
    ...
    nome_arq = msg.decode('ascii')  #arquivo precisa estar na mesma pasta do servidor.
    ...
    if os.path.isfile(nome_arq):
        ...
        # == obs: não é preciso o cliente enviar uma msg para q o servidor encvie uma resposta.

        # Abre o arquivo no modo leitura de bytes
        arq = open(nome_arq, 'rb')
        
        # Envia os dados
        #obs.: É entendido automaticamente o envio de bloco por bloco de 4096 bytes
        bytes = arq.read(4096)
        while bytes:
